...
/** Handler for OrderCreatedEvent Processes the event and creates a payment */
@Component
...
public class OrderCreatedEventHandler {
  ...
  public void handle(OrderCreatedEvent event) {
    log.info(
        "Processing OrderCreatedEvent: orderId={}, userId={}, amount={}",
        event.getOrderId(),
        event.getUserId(),
    ...
    try {
      ...
      if (!result.isSuccess()) {
        ...
        throw new PaymentCreationException("Failed to create payment: " + result.getErrorMessage());
      }

      log.info(
          "Payment created successfully for order: {}, paymentId: {}",
          event.getOrderId(),
          result.getData().getPaymentId());
