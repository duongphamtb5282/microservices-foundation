package com.pacific.payment.filter;
...
public class CorrelationIdFilter extends OncePerRequestFilter {
     ...
     * This method is defensive against null/empty/malformed headers
     */
    private String extractOrGenerateCorrelationId(HttpServletRequest request) {
        try {
            ...
            if (trimmedValue.length() < 5 || trimmedValue.length() > 100) {
                ...
                // log.warn("Suspicious correlation ID length: {}, generating new one", trimmedValue.length());
                return UUID.randomUUID().toString();
            }
