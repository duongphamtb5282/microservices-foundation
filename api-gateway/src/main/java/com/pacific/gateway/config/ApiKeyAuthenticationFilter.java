...
/**
 * Gateway filter for API key authentication (alternative to JWT). Validates API keys for
 * service-to-service communication.
 */
@Component
@Slf4j
public class ApiKeyAuthenticationFilter
    extends AbstractGatewayFilterFactory<ApiKeyAuthenticationFilter.Config> {
  ...
  public ApiKeyAuthenticationFilter(
      SecurityService securityService, WebClient.Builder webClientBuilder) {
    super(Config.class);
  ...
  public GatewayFilter apply(Config config) {
    return (exchange, chain) -> {
      ...
      if (apiKey == null || apiKey.isEmpty()) {
        ...
        byte[] errorBytes =
            "{\"error\":\"Missing API Key\",\"code\":\"MISSING_API_KEY\"}".getBytes();
        return exchange
            .getResponse()
      ...
      if (!securityService.validateApiKey(apiKey)) {
        ...
        exchange.getResponse().setStatusCode(HttpStatus.UNAUTHORIZED);
        byte[] errorBytes =
            "{\"error\":\"Invalid API Key\",\"code\":\"INVALID_API_KEY\"}".getBytes();
        return exchange
            .getResponse()
            .writeWith(Mono.just(exchange.getResponse().bufferFactory().wrap(errorBytes)));
      }
