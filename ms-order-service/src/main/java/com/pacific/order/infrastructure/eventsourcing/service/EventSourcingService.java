...
/**
 * Service for event sourcing operations on Order aggregates. Provides capabilities to rebuild
 * aggregates from events and query event data.
 */
@Service
...
public class EventSourcingService {
  ...
  public Optional<OrderAggregate> rebuildAggregateFromVersion(String orderId, int fromVersion) {
    try {
      List<OrderDomainEvent> events =
          eventStoreRepository.getEventsForAggregateFromVersion(orderId, fromVersion);
      ...
      log.debug(
          "Rebuilt aggregate: {} from version: {} with {} events",
          orderId,
          fromVersion,
          events.size());
      return Optional.of(aggregate);
  ...
  public static class EventSourcingResult {
    ...
    // Getters
    public List<OrderDomainEvent> getEvents() {
      return events;
    }
    ...
    public static class EventSourcingResultBuilder {
      ...
      public EventSourcingResult build() {
        return new EventSourcingResult(
            events, totalEvents, page, size, totalPages, hasNext, hasPrevious);
      }
    }
  }
  ...
  public static class EventStatistics {
    ...
    public java.util.Map<String, Long> getEventsByType() {
      return eventsByType;
    }
