...
public interface OrderEventJpaRepository extends JpaRepository<OrderEventEntity, String> {
  ...
  @Query(
      "SELECT e FROM OrderEventEntity e WHERE e.orderId = :orderId AND e.version >= :fromVersion ORDER BY e.version ASC")
  List<OrderEventEntity> findByOrderIdFromVersionOrderByVersionAsc(
      @Param("orderId") String orderId, @Param("fromVersion") int fromVersion);
  ...
  List<OrderEventEntity> findByUserIdAndEventTypeOrderByEventTimestampDesc(
      String userId, String eventType);

  /** Find events within a time range */
  @Query(
      "SELECT e FROM OrderEventEntity e WHERE e.eventTimestamp BETWEEN :startTime AND :endTime ORDER BY e.eventTimestamp ASC")
