...
public class CancelOrderCommandHandler
    implements CommandHandler<CancelOrderCommand, OrderResponse> {
  ...
  public CommandResult<OrderResponse> handle(CancelOrderCommand command) {
    try {
      ...
      if (!order.getUserId().equals(command.getUserId())) {
        return CommandResult.failure("Order does not belong to user", "ORDER_ACCESS_DENIED");
      }
      ...
      OrderCancelledEvent eventSourcingEvent =
      ...
      businessMetricsService.recordOrderCancelled(savedOrder.getUserId(), command.getReason());
